...
public class BraveNtpTopSitesModeUnitTest {
    ...
    // ---- BraveNtpDelegate item ID safety ----

    /**
     * Brave item IDs must be >= ContextMenuItemId.NUM_ENTRIES (18) so they never shadow a
     * current or future Chromium value in a switch statement.
     */
    @Test
    ...
    public void testBraveItemIds_areAllUnique() {
        ...
        for (int i = 0; i < ids.length; i++) {
            for (int j = i + 1; j < ids.length; j++) {
                assertFalse(
                        "IDs at index " + i + " and " + j + " must be unique",
                        ids[i] == ids[j]);
            }
        }
    }

    // ---- Mode constants match delegate checkmark logic ----

    /**
     * The checkmark logic in the patch uses 0 == SHORTCUTS and 1 == FREQUENT. These tests
     * catch accidental reordering of the constants.
     */
    @Test
