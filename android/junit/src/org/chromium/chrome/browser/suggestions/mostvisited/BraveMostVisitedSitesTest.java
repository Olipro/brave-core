...
public class BraveMostVisitedSitesTest {
    ...
    public void setUp() {
        ...
        ArgumentCaptor<MostVisitedSites.Observer> captor = forClass(MostVisitedSites.Observer.class);
    ...
    public void testFilterForMode_shortcuts_keepsOnlyCustomLinks() {
        ...
        assertTrue("All filtered tiles must be CUSTOM_LINKS", allMatch(result, TileSource.CUSTOM_LINKS));
    ...
    public void testFilterForMode_shortcuts_emptyWhenNoCustomLinks() {
        List<SiteSuggestion> topSitesOnly = List.of(
                makeSuggestion("Example", "https://example.com", TileSource.TOP_SITES));
        List<SiteSuggestion> result =
    ...
    public void testFilterForMode_frequent_emptyWhenNoTopSites() {
        List<SiteSuggestion> customOnly = List.of(
                makeSuggestion("My Site", "https://mysite.com", TileSource.CUSTOM_LINKS));
        List<SiteSuggestion> result =
    ...
    // ---- FilteringObserver integration ----

    @Test
    public void testBridgeCallbackFiltersForCurrentMode_defaultShortcutsMode() {
        // Default mode is SHORTCUTS; only CUSTOM_LINKS tiles should reach the outer observer.
    ...
    public void testModeChangeRefiltersCachedTiles() {
        ...
        verify(mMockOuterObserver, times(2)).onSiteSuggestionsAvailable(anyBoolean(), captor.capture());
        List<SiteSuggestion> secondCall = captor.getAllValues().get(1);
        assertTrue("After mode change, only TOP_SITES tiles", allMatch(secondCall, TileSource.TOP_SITES));
    ...
    public void testSwitchingModesTwice_correctFinalState() {
        ...
        ArgumentCaptor<List<SiteSuggestion>> captor = captorForSuggestions();
        verify(mMockOuterObserver, times(3)).onSiteSuggestionsAvailable(anyBoolean(), captor.capture());

        List<SiteSuggestion> thirdCall = captor.getAllValues().get(2);
        assertTrue("Final mode is SHORTCUTS, expect CUSTOM_LINKS only",
                allMatch(thirdCall, TileSource.CUSTOM_LINKS));
    }

    // ---- Helpers ----

    /** Three tiles: one CUSTOM_LINKS and two TOP_SITES. */
    private static List<SiteSuggestion> mixedSuggestions() {
        List<SiteSuggestion> list = new ArrayList<>();
        list.add(makeSuggestion("My shortcut", "https://shortcut.example.com", TileSource.CUSTOM_LINKS));
    ...
    private static SiteSuggestion makeSuggestion(String title, String url, int source) {
        // titleSource=0 and sectionType=0 (PERSONALIZED) are not relevant to the filtering tests.
        return new SiteSuggestion(title, new GURL(url), /* titleSource= */ 0, source,
                /* sectionType= */ 0);
    }
