 ...
 * </ul>
 *
 * <p>Filtering in Java rather than by changing {@code EnableTileTypes()} in C++ avoids the
 * {@code ShouldQueryTopSites()} fallback issue: when custom links are not yet initialised the C++
 * layer always queries top-sites regardless of the tile-type options, so both modes would receive
...
public class BraveMostVisitedSites implements MostVisitedSites {
    ...
    public BraveMostVisitedSites(Profile profile) {
        this(
                new MostVisitedSitesBridge(
                        profile, /* enableCustomLinks= */ true));
    }

    /**
     * Test-only constructor that accepts an injected {@link MostVisitedSites} so the JNI bridge
     * can be replaced with a mock or fake.
     */
    @VisibleForTesting
    ...
    // Delegate all read-path and write-path calls straight to the bridge.
    ...
    /** Filters {@code all} down to the tiles that should be visible in the current mode. */
    @VisibleForTesting
    ...
    // Intercepts bridge callbacks to apply mode filtering and cache the raw tile list.
